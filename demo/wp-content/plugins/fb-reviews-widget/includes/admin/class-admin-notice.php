...
class Admin_Notice {
    ...
    public function remove_query_args($args) {
        return array_merge($args, array(Plugin::SLG . '_notice'));
    }
    ...
    public function parse_notices_from_url() {
        ...
        $this->notice_id = sanitize_text_field(wp_unslash($_GET[Plugin::SLG . '_notice']));
    }
    ...
    public function render_notices() {
        ...
        if (doing_action('admin_notices') && $this->needs_repositioned()) {
            add_action(Plugin::SLG . '_admin_notices', array($this, 'render_notices'));
            return;
        }

        ?>
        <div class="notice notice-<?php echo get_option(Plugin::SLG . '_notice_type'); ?> is-dismissible">
            <p><?php echo $this->notice_id != 'custom_msg' ? self::$plugin_notices[$this->notice_id] : get_option(Plugin::SLG . '_notice_msg'); ?></p>
        </div>
        <?php
    ...
    protected function is_valid_screen($screen_id = '') {
        ...
        return ($screen_id === 'dashboard' || $screen_id === 'plugins' || strpos($screen_id, Plugin::SLG) !== false) ? true : false;
    }
    ...
    protected function needs_repositioned($screen_id = '') {
        ...
        $screen_ids = array('trust-reviews_page_' . Plugin::SLG . '-settings');
