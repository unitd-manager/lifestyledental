...
<form accept-charset="UTF-8" action="#" class="infusion-form gradient-form" method="POST" id="teeth-whitening">
    <input name="inf_form_xid" value="72d3f9c290dfe79b694fac249dcdafb1" type="hidden">
    <input name="inf_form_name" value="Braces Page Inquiry" type="hidden">
...
<script>
    var teethWhiteningForm = document.querySelector("form#teeth-whitening");
    teethWhiteningForm.addEventListener("submit", function(e){
        e.preventDefault();

        if (teethWhiteningForm.querySelector('#sirname').value == "") {
                ...
                grecaptcha.execute('6LfLgnIcAAAAAIcmAGVwRtd52GZAUjTKMsQBwCPC', {action: 'submit'}).then(function(token) {
                    sendToTheGoogleGodToVerify(token).then(function(result){
                        if (result.success){
                            ...
                            teethWhiteningForm.action = 'https://lifestyledental.infusionsoft.com/app/form/process/72d3f9c290dfe79b694fac249dcdafb1';

                            teethWhiteningForm.submit();
                        }
                    });
                });
            });
