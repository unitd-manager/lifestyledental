...
class Admin_Feed_Columns {
    ...
    public function register() {
        ...
        add_filter('get_the_excerpt', array($this, 'hide_feed_excerpt'), 10, 2);
    }
    ...
    public function change_edit_post_link($link, $id, $context) {
        if (function_exists('get_current_screen')) {
            ...
            return admin_url('admin.php?page=' . Plugin::SLG . '-builder&' . Post_Types::FEED_POST_TYPE . '_id=' . $id);
        } else {
            return;
        }
    }
    ...
    public function get_columns($columns) {
        ...
        $columns = array(
            ...
            'ID'        => __('ID',    Plugin::NAME),
            Plugin::SLG . '_theme' => __('Theme', Plugin::NAME),
    ...
    public function change_post_row_actions($actions, $post) {
        if (isset($actions) && $post->post_type === Post_Types::FEED_POST_TYPE) {
            $changed_actions = array(
                'post-id' => '<span class="' . Plugin::SLG . '-admin-column-action">ID: ' . $post->ID . '</span>',
            );
    ...
    public function hide_feed_excerpt($excerpt, $post = null) {
