...
class View {
    ...
    function place($rating, $place, $place_img, $reviews, $options, $show_powered = true, $show_writereview = false) {
        ...
        <div class="{slg}-left">
            <img src="<?php echo esc_url($place_img); ?>" alt="<?php echo esc_attr($place->name); ?>" width="50" height="50" title="<?php echo esc_attr($place->name); ?>">
        </div>
        <?php } ?>
        <div class="{slg}-right">
    ...
    function stars($rating) {
        foreach (array(1,2,3,4,5) as $val) {
            $score = $rating - $val;
            if ($score >= 0) {
                ?><svg width="22" height="22" viewBox="0 0 24 24"><use xlink:href="#{slg}-star"/></svg><?php
            } else if ($score > -1 && $score < 0) {
                ?><svg width="22" height="22" viewBox="0 0 24 24"><use xlink:href="#{slg}-star-o" style="stroke:currentColor;stroke-width:1.5"/><use href="#{slg}-star-half" clip-path="url(#{slg}-clip-half)"/></svg><?php
            } else {
                ?><svg width="22" height="22" viewBox="0 0 24 24"><use xlink:href="#{slg}-star-o" style="stroke:#ccc;stroke-width:1.5"/></svg><?php
            }
        }
    }
    ...
    function image($src, $alt, $lazy, $def_ava = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7', $atts = '') {
        $src = esc_url($src);
        $alt = esc_attr($alt);
        $def_ava = esc_url($def_ava);
        ?><img src="<?php echo $src; ?>" <?php if ($lazy) { ?>loading="lazy"<?php } ?> alt="<?php echo $alt; ?>" onerror="this.onerror=null;this.src='<?php echo $def_ava; ?>';" <?php echo $atts; ?>><?php
    }
